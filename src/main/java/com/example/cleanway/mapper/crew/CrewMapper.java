...
public interface CrewMapper {
    // 크루 모집 리스트(홈)
      List<CrewVo> selectCrewList();
//    크루 리스트 서치(홈)
     List<CrewVo> selectCrewByWord(String searchWord);
    //크루 모집 글쓰기
//    크루 작성자 = 자동 크루장
//    크루 모집인원 = 최초 프로젝트 일치
     void crewInsert(CleanCrewDto cleanCrewDto);
//크루 모집 상세보기
...
Long getCrewProjectNumber(@Param("crewNumber") Long crewNumber, @Param("crewName") String crewName);
    //    크루 참여
     void crewJoinInsert(CleanMyCrewDto cleanMyCrewDto);
// 특정 회원이 특정 크루에 참여했는지 검사 여부
    int selectMyCrewJoinCount (CleanMyCrewDto cleanMyCrewDto);
 //내 크루 목록 보기
     List<MyCrewVo> selectMyCrewList(Long userNumber);
//    내 크루 검색어 써치
     List<MyCrewVo> selectMyCrewByWord(Long userNumber,String searchWord);
